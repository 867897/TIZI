...
#=================================================
#	System Required: CentOS 7+/Debian 9+/Ubuntu 18.04+
#	Description: Install the ShadowsocksR server (hardened fork)
#	Version: 2.0.38-hardened
#=================================================

sh_ver="2.0.38-hardened"

#----- 镜像源配置（已指向 867897/TIZI 仓库的 ssr/ 子目录）-----
# 仓库结构：
#   867897/TIZI (branch: main)
#     └─ ssr/
#         ├─ SSR/shadowsocksr-manyuser.zip
#         ├─ service/ssr_centos
#         ├─ service/ssr_debian
#         ├─ ban_iptables.sh
#         └─ bbr.sh
MIRROR_OWNER="867897"
MIRROR_REPO="TIZI"
MIRROR_REF="main"          # 当前资源由 main 分支直接维护，需与已上传 ZIP 保持一致
MIRROR_SUBDIR="ssr"        # 仓库内的子目录
SELF_UPDATE_REF="main"

# 资源 URL（统一从你自己的仓库拉取，HTTPS）
RAW_BASE="https://raw.githubusercontent.com/${MIRROR_OWNER}/${MIRROR_REPO}/${MIRROR_REF}/${MIRROR_SUBDIR}"
SSR_SRC_URL="${RAW_BASE}/SSR/shadowsocksr-manyuser.zip"
SSR_SRC_INNER_DIR="shadowsocksr-manyuser"   # zip 解压后的顶层目录名
SSR_INIT_CENTOS_URL="${RAW_BASE}/service/ssr_centos"
SSR_INIT_DEBIAN_URL="${RAW_BASE}/service/ssr_debian"
BBR_SH_URL="${RAW_BASE}/bbr.sh"
BAN_IPTABLES_URL="${RAW_BASE}/ban_iptables.sh"
SELF_UPDATE_URL="https://raw.githubusercontent.com/${MIRROR_OWNER}/${MIRROR_REPO}/${SELF_UPDATE_REF}/${MIRROR_SUBDIR}/ssr.sh"   # 自更新跟踪 main

# 可选：为关键下载提供 SHA-256 校验。强烈建议在你本地算一次后填进来。
# 算法： curl -fSL <URL> | sha256sum    把第一段 hash 粘到这里
SSR_SRC_SHA256="38a5c8673dc708c0d0fa88fdf2290786d56912eb2bbde18afcf9cb430579745b"
SSR_INIT_CENTOS_SHA256="713114ae6725f8ae4bb18b948ff1cdb89c6fc0c76f17ea799500ed43a387344c"
SSR_INIT_DEBIAN_SHA256="ac50be3acd1daf83e0b81f8c3616fbe4e7b56a63b0adaabc773932739d8a3343"
BBR_SH_SHA256="1867f03ba8d9fc84c1d5ef368c3cc91f8ff7b26980f78c7c319fe1fdf162d7f5"
BAN_IPTABLES_SHA256="6ef88cda63af3c8762eccc5d23f228f7b2b591fcf5da839cc2760620d5c7fb91"
SELF_UPDATE_SHA256=""   # 等你把改好的 ssr.sh 上传到仓库后再算: sha256sum ssr.sh

# 锐速 / LotServer 是商业闭源内核模块，上游脚本本身不可验证 + 长期未维护；默认 禁用。
ALLOW_LEGACY_SPEEDER=0   # 设为 1 才允许安装 serverspeeder/lotserver
SPEEDER_91YUN_URL=""     # 如需启用，请填你自己 fork 的 raw 地址
SPEEDER_91YUN_SHA256=""
LOTSERVER_URL=""
LOTSERVER_SHA256=""
#-----------------------------------------------------------

# 安全下载工具：强制 TLS 校验 + 失败重试；可选 SHA-256 校验
safe_download(){
	...
	if [[ -n "${sum}" ]]; then
		...
		[[ "${actual}" == "${sum}" ]] || {
			echo -e "${Error:-[ERROR]} 文件校验失败: ${dest}" >&2
			echo -e "${Error:-[ERROR]} 期望 SHA256: ${sum}" >&2
			echo -e "${Error:-[ERROR]} 实际 SHA256: ${actual}" >&2
			echo -e "${Error:-[ERROR]} 下载地址: ${url}" >&2
			rm -f "${dest}"
			return 2
		}
	fi
...
check_pid(){
	# 严格匹配：只命中安装目录里的 server.py，避免误杀别的 python 进程
	if command -v pgrep >/dev/null 2>&1; then
		PID=$(pgrep -f "${ssr_folder}/.*server\.py" | head -n1)
	else
		PID=$(ps -eo pid,cmd --no-headers 2>/dev/null | awk -v f="${ssr_folder}" '$0 ~ f"/.*server\\.py" {print $1; exit}')
	fi
	[[ ! "${PID}" =~ ^[0-9]+$ ]] && PID=""
}
...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! safe_download "${BBR_SH_URL}" "bbr.sh" "${BBR_SH_SHA256}"; then
...
# 设置 防火墙规则（幂等 + 变量加引号 + 修复 port/ssr_port bug）
Add_iptables(){
	[[ -z "${ssr_port}" ]] && { echo -e "${Error} ssr_port 为空，拒绝写入 iptables"; return 1; }
	local ipt
	for ipt in iptables ip6tables; do
		for proto in tcp udp; do
			${ipt} -C INPUT -m state --state NEW -m "${proto}" -p "${proto}" --dport "${ssr_port}" -j ACCEPT 2>/dev/null \
				|| ${ipt} -I INPUT -m state --state NEW -m "${proto}" -p "${proto}" --dport "${ssr_port}" -j ACCEPT
		done
	done
}
Del_iptables(){
	local _port="${ssr_port:-${port}}"
	[[ -z "${_port}" ]] && { echo -e "${Error} 端口为空，跳过 iptables 删除"; return 0; }
	local ipt
	for ipt in iptables ip6tables; do
		for proto in tcp udp; do
			while ${ipt} -C INPUT -m state --state NEW -m "${proto}" -p "${proto}" --dport "${_port}" -j ACCEPT 2>/dev/null; do
				${ipt} -D INPUT -m state --state NEW -m "${proto}" -p "${proto}" --dport "${_port}" -j ACCEPT
			done
		done
	done
}
...
# 读取 配置信息（HTTPS + 校验返回值是 IP 形式）
Get_IP(){
	local candidates=( "https://api.ipify.org" "https://ipinfo.io/ip" "https://api.ip.sb/ip" )
	local raw
	for u in "${candidates[@]}"; do
		raw=$(wget --https-only -qO- -t1 -T3 "$u" 2>/dev/null | tr -d '\r\n[:space:]')
		if [[ "${raw}" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ || "${raw}" =~ ^[0-9a-fA-F:]+$ ]]; then
			ip="${raw}"
			return 0
		fi
	done
...
# 本地生成二维码，避免把含密码的链接发给第三方域名
render_qr(){
	# usage: render_qr <text>
	if command -v qrencode >/dev/null 2>&1; then
		qrencode -t ANSIUTF8 -m 2 -- "$1"
	else
		echo " (未安装 qrencode，无法本地渲染二维码；可执行: apt-get install -y qrencode  或  yum install -y qrencode)"
	fi
}
ss_link_qr(){
...
Set_config_password(){
	local default_pwd
	default_pwd=$(head -c 18 /dev/urandom 2>/dev/null | base64 | tr -d '+/=\n' | cut -c1-24)
	[[ -z "${default_pwd}" ]] && default_pwd=$(date +%s%N | sha256sum | cut -c1-24)
	echo "请输入要设置的ShadowsocksR账号 密码（留空将使用随机生成的强密码）"
	read -e -p "(默认: ${default_pwd}):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="${default_pwd}"
...
Set_config_method(){
 ...
 ${Green_font_prefix}4.${Font_color_suffix} chacha20-ietf        ${Tip} 移动端/低性能 CPU 推荐（需 libsodium）
 ${Green_font_prefix}5.${Font_color_suffix} chacha20             ${Tip} 需 libsodium
 ${Green_font_prefix}6.${Font_color_suffix} none                 ${Tip} 仅在 protocol=auth_chain_* 时使用" && echo
	read -e -p "(默认: 1. aes-256-cfb):" ssr_method
	[[ -z "${ssr_method}" ]] && ssr_method="1"
	case "${ssr_method}" in
		1) ssr_method="aes-256-cfb" ;;
		2) ssr_method="aes-192-cfb" ;;
		3) ssr_method="aes-128-cfb" ;;
		4) ssr_method="chacha20-ietf" ;;
		5) ssr_method="chacha20" ;;
		6) ssr_method="none" ;;
...
Patch_python_compat(){
	local file_path="$1" anchor_line="$2" replace_from="$3" replace_to="$4"
	local tmp_file
	[[ ! -f "${file_path}" ]] && return 0
	tmp_file="${file_path}.tmp"
	awk -v anchor="${anchor_line}" '
		$0 ~ /^from collections\.abc import MutableMapping$/ { next }
		$0 ~ /^from collections import MutableMapping$/ { next }
		{ print }
		$0 == anchor {
			print ""
			print "try:"
			...
			print "    from collections import MutableMapping"
		}
	' "${file_path}" > "${tmp_file}" && mv "${tmp_file}" "${file_path}"
	[[ -n "${replace_from}" ]] && sed -i "s/${replace_from}/${replace_to}/" "${file_path}"
}
Centos_yum(){
	yum update
...
# 下载 ShadowsocksR（强制 HTTPS 校验 + 可选 SHA-256 校验）
Download_SSR(){
	cd "/usr/local/"
	local zip_name="shadowsocksr-manyuser.zip"
	local extracted_dir="${SSR_SRC_INNER_DIR}"
	if ! safe_download "${SSR_SRC_URL}" "${zip_name}" "${SSR_SRC_SHA256}"; then
		echo -e "${Error} ShadowsocksR 源码下载失败 !" && rm -f "${zip_name}" && exit 1
	fi
	unzip -q "${zip_name}"
	[[ ! -d "/usr/local/${extracted_dir}" ]] && echo -e "${Error} ShadowsocksR服务端 解压失败 (期望目录: ${extracted_dir}) !" && rm -f "${zip_name}" && exit 1
	mv "/usr/local/${extracted_dir}" "/usr/local/shadowsocksr"
	[[ ! -e "/usr/local/shadowsocksr/" ]] && echo -e "${Error} ShadowsocksR服务端 重命名失败 !" && rm -f "${zip_name}" && exit 1
	rm -f "${zip_name}"
	[[ -e ${config_folder} ]] && rm -rf ${config_folder}
	mkdir ${config_folder}
	[[ ! -e ${config_folder} ]] && echo -e "${Error} ShadowsocksR配置文件的文件夹 建立失败 !" && exit 1
    # 修复 Ubuntu 22.04 / OpenSSL 3.0 问题
    if [[ -f "/usr/local/shadowsocksr/shadowsocks/crypto/openssl.py" ]]; then
        sed -i "s/EVP_CIPHER_CTX_cleanup/EVP_CIPHER_CTX_reset/g" /usr/local/shadowsocksr/shadowsocks/crypto/openssl.py
    fi
	# 修复 Python 3.10+/3.11 collections.MutableMapping 兼容问题
	Patch_python_compat "/usr/local/shadowsocksr/shadowsocks/lru_cache.py" "import time" "class LRUCache(collections\\.MutableMapping):" "class LRUCache(MutableMapping):"
	Patch_python_compat "/usr/local/shadowsocksr/shadowsocks/ordereddict.py" "import collections" "update = collections\\.MutableMapping\\.update" "update = MutableMapping.update"
	echo -e "${Info} ShadowsocksR服务端 下载完成 !"
}
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! safe_download "${SSR_INIT_CENTOS_URL}" "/etc/init.d/ssr" "${SSR_INIT_CENTOS_SHA256}"; then
		...
		if ! safe_download "${SSR_INIT_DEBIAN_URL}" "/etc/init.d/ssr" "${SSR_INIT_DEBIAN_SHA256}"; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
...
Check_Libsodium_ver(){
	echo -e "${Info} 使用已知良好的 libsodium 版本 ${Libsodiumr_ver_backup}（不再自动抓取最新版以避免被中间人替换）"
...
Install_Libsodium(){
	...
	if [[ ${release} == "centos" ]]; then
		...
		echo -e "${Info} 下载..."
		safe_download "https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}-RELEASE/libsodium-${Libsodiumr_ver}.tar.gz" "libsodium-${Libsodiumr_ver}.tar.gz" "" \
			|| { echo -e "${Error} libsodium 下载失败"; exit 1; }
		echo -e "${Info} 解压..."
		...
		apt-get update
		echo -e "${Info} 尝试使用 apt 安装 libsodium..."
		apt-get install -y libsodium-dev
		if [[ $? -eq 0 && -e /usr/lib/x86_64-linux-gnu/libsodium.so ]]; then
			echo -e "${Info} apt 安装成功，创建链接..."
			ln -sf /usr/lib/x86_64-linux-gnu/libsodium.so /usr/local/lib/libsodium.so
		else
			echo -e "${Info} apt 安装失败或未找到文件，尝试编译安装..."
			echo -e "${Info} 安装依赖..."
			apt-get install -y build-essential
			echo -e "${Info} 下载..."
			safe_download "https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}-RELEASE/libsodium-${Libsodiumr_ver}.tar.gz" "libsodium-${Libsodiumr_ver}.tar.gz" "" || {
				echo -e "${Error} 下载失败，尝试使用备用版本 ${Libsodiumr_ver_backup} ..."
				Libsodiumr_ver=${Libsodiumr_ver_backup}
				safe_download "https://github.com/jedisct1/libsodium/releases/download/${Libsodiumr_ver}-RELEASE/libsodium-${Libsodiumr_ver}.tar.gz" "libsodium-${Libsodiumr_ver}.tar.gz" "" || { echo -e "${Error} libsodium 下载失败"; exit 1; }
			}
			echo -e "${Info} 解压..."
			tar -xzf libsodium-${Libsodiumr_ver}.tar.gz && cd libsodium-${Libsodiumr_ver}
			echo -e "${Info} 编译安装..."
...
Install_ServerSpeeder(){
	...
	if [[ "${ALLOW_LEGACY_SPEEDER}" != "1" ]]; then
		echo -e "${Error} 锐速(91yun 开心版) 是商业闭源内核模块，上游脚本多年未维护，本脚本默认禁用。请改用 BBR / BBRplus。如仍需使用，请在脚本顶部将 ALLOW_LEGACY_SPEEDER 设为 1。" && exit 1
	fi
	cd /root
	if ! safe_download "${SPEEDER_91YUN_URL}" "serverspeeder.sh" "${SPEEDER_91YUN_SHA256}"; then
		echo -e "${Error} 锐速安装脚本下载失败 !" && exit 1
	fi
	bash serverspeeder.sh
	sleep 2s
	PID=$(pgrep -f serverspeeder | head -n1)
	if [[ ! -z ${PID} ]]; then
...
Install_LotServer(){
	...
	if [[ "${ALLOW_LEGACY_SPEEDER}" != "1" ]]; then
		echo -e "${Error} LotServer / 锐速 是商业闭源内核模块，上游脚本多年未维护，本脚本默认禁用。请改用 BBR / BBRplus / TCP optimization。如仍需使用，请在脚本顶部将 ALLOW_LEGACY_SPEEDER 设为 1。" && exit 1
	fi
	if ! safe_download "${LOTSERVER_URL}" "/tmp/appex.sh" "${LOTSERVER_SHA256}"; then
		echo -e "${Error} LotServer 安装脚本下载失败 !" && exit 1
	fi
	bash /tmp/appex.sh 'install'
	sleep 2s
	local LSPID
	LSPID=$(pgrep -f appex | head -n1)
	if [[ -n "${LSPID}" ]]; then
...
# 封禁 BT PT SPAM
BanBTPTSPAM(){
	if ! safe_download "${BAN_IPTABLES_URL}" "ban_iptables.sh" "${BAN_IPTABLES_SHA256}"; then
		echo -e "${Error} ban_iptables.sh 下载失败" && return 1
	fi
	chmod +x ban_iptables.sh && bash ban_iptables.sh banall
...
UnBanBTPTSPAM(){
	...
	chmod +x ban_iptables.sh && bash ban_iptables.sh unbanall
	rm -f ban_iptables.sh
}
Set_config_connect_verbose_info(){
	SSR_installation_status
...
Update_Shell(){
	local tmp_new="/tmp/ssr.sh.new"
	if ! safe_download "${SELF_UPDATE_URL}" "${tmp_new}" "${SELF_UPDATE_SHA256}"; then
		echo -e "${Error} 无法下载脚本新版本 !" && exit 1
	fi
	sh_new_ver=$(grep 'sh_ver="' "${tmp_new}" | head -1 | awk -F '"' '{print $2}')
	[[ -z "${sh_new_ver}" ]] && echo -e "${Error} 新脚本格式异常，拒绝覆盖" && rm -f "${tmp_new}" && exit 1
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	mv "${tmp_new}" "./ssr.sh" && chmod +x ssr.sh
	echo -e "脚本已更新为 [ ${sh_new_ver} ] !" && exit 0
}
# 显示 菜单状态
menu_status(){
